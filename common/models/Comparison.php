...
//    print_r($changedAttributes);
    ...
    if ($res_comparison) $res = 'SHOW';
    else{
      if ((int)$count === (int)$res_all_comparison) $res = 'HIDE';
    }
...
/**/
    ...
    $all_compared = false;
    if ((int)$count === (int)$res_all_comparison_2) $all_compared = true;
    ...
    if ($all_compared){
      ...
      // запись есть? → удаляем
      if ($find_2) P_all_compare::findOne(['p_id' => $p_id,'source_id' => $source_id])->delete();
    }
