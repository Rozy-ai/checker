...
class Product_right extends \yii\base\DynamicModel {
    ...
    public function __construct(Source $source, array $attributes = [], $config = []) {
        $this->source = $source;
    ...
    private function calculate_step_2(array $array) {
        ...
        if (is_array($v_1))
            $v_1 = $this->calculate_step_2($v_1);
        if (is_array($v_2))
            $v_2 = $this->calculate_step_2($v_2);
    ...
    public function __get($name) {
        ...
        $d = $this->source->getDataFields($name);
...
//      exit;
        ...
        if ($d['settings__source_fields_field_action'] == 'replace') {
            ...
            foreach ($matches as $m) {
                ...
                if (!$v = $this->get_($m[1])) {
                    ...
                    $v = $parent_item[$m[1]] ?: '';
                    if ($v_type === 'int')
                        $v = (int) $parent_item[$m[1]] ?: 0;
                    if ($v_type === 'float')
                        $v = (float) $parent_item[$m[1]] ?: 0;
                };
        ...
        if ($this->hasAttribute($d['settings__source_fields_name']) && $d['settings__source_fields_name'])
            return $p_name = parent::__get($d['settings__source_fields_name']);
        else {
