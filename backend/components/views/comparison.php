    ...
    <?php if ($model->canCompare): ?>
        <?php if (0 && $model->comparison): ?>
            ...
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="btnGroupDrop2">
                <?= Html::a(Yii::t('site', Comparison::STATUS_MATCH), ['compare', 'id' => $product_id, 'node' => $node_idx, 'status' => Comparison::STATUS_MATCH], ['class' => 'dropdown-item']) ?>
                <?= Html::a(Yii::t('site', Comparison::STATUS_MISMATCH), ['compare', 'id' => $product_id, 'node' => $node_idx, 'status' => Comparison::STATUS_MISMATCH, 'return' => 1], ['class' => 'dropdown-item']) ?>
                <div class="dropdown-divider"></div>
                <h6 class="dropdown-header"><?= Yii::t('site', Comparison::STATUS_OTHER) ?></h6>
                <?php foreach ($messages as $message): ?>
                    <?= Html::a($message->text, ['compare', 'id' => $product_id, 'msgid' => $message->id, 'node' => $node_idx, 'status' => Comparison::STATUS_OTHER], ['class' => 'dropdown-item']) ?>
                <?php endforeach; ?>
            </div>
        ...
        ?>
            ...
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="btnGroupDrop2">
                <?php foreach ($messages as $message): ?>
                    <?= Html::a($message->text, ['compare', 'id' => $product_id, 'source_id' => $source_id, 'msgid' => $message->id, 'node' => $node_idx, 'status' => Comparison::STATUS_OTHER], ['class' => 'dropdown-item']) ?>
                <?php endforeach; ?>
            </div>
