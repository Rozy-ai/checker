...
/* @var $list backend\controllers\ProductController */
/* @var $cnt_all backend\controllers\ProductController */
/* @var $on_page backend\controllers\ProductController */
/* @var $pages_cnt backend\controllers\ProductController */
/* @var $url_construct backend\controllers\ProductController */
/* @var $page_n backend\controllers\ProductController */
/* @var $get_ backend\controllers\ProductController */
/* @var $where_3_list backend\controllers\ProductController */
/* @var $where_4_list backend\controllers\ProductController */
/* @var $where_6_list backend\controllers\ProductController */
/* @var $is_admin backend\controllers\ProductController */
/* @var $no_compare backend\controllers\ProductController */
/* @var $pager backend\controllers\ProductController */
/* @var $sort backend\controllers\ProductController */
/* @var $right_item_show backend\controllers\ProductController */
/* @var $profiles_list backend\controllers\ProductController */
/* @var $last_update backend\controllers\ProductController */
      ...
      <form action="" class="products__filter-form" >
        ...
        <div class="form-row js-title-and-source_selector">
          ...
          <select name="filter-items__source" id="filter-items__source" class="form-control">
            <? if ($sources = \backend\models\Source::get_sources()): ?>
              <? foreach ($sources as $source): ?>
                <option
                  value="<?=$source->id?>"
                  <? if ($source_id === (int)$source->id) { echo 'selected="selected"'; $selected_source_name = $source->name;} ?>
                ><?=$source->name?></option>
              <? endforeach;?>
            <? endif; ?>
          ...
          <div class="form-group _col-sm-2" >
            <select name="filter-items__profile" id="filter-items__profile" class="form-control ">
              <? foreach ($profiles_list as $k_profile => $profile):?>
                <option value="<?=$k_profile?>" <?=($k_profile === $get_['filter-items__profile'])? 'selected' : ''?>> <?=$profile?> </option>
              <? endforeach; ?>
            </select>
          </div>
          <? endif; ?>
        ...
        <div class="form-row form-inline" style="width: 100%;">
          ...
          ?>
          <div class="cnt-items col-sm-6">Показаны записи <?=$on_page?> из <?=$cnt_all;?>. Источник <?=$selected_source_name?> / <?= $profile_path?></div>
          ...
          <div class="cnt-items col-sm-6" style="    text-align: right; padding-right: 0;">
            ...
            <select name="filter-items__show_n_on_page" id="filter-items__show_n_on_page" class="form-control ">
              <? foreach ($pages_n_list as $pnl):?>
                <option value="<?=$pnl?>" <?=((int)$get_['filter-items__show_n_on_page'] === $pnl)? 'selected' : ''?>><?=$pnl?></option>
              <? endforeach;?>
                <option value="ALL" <?= ($get_['filter-items__show_n_on_page'] === 'ALL')? 'selected' : '' ?> >ВСЕ</option>
            </select>
          </div>
        </div>
        ...
        <div class="form-row">
          ...
          <? if (User::is_detail_view_for_items() || User::isAdmin()): ?>
          <div class="form-group _col-sm-3" >
  ...
  <table class="table table-striped [ PRODUCT-LIST ] products__products-list">
    ...
    >
      <td class="products-list__td1">
        ...
        <div class="product-list-item__data"><span>id:</span><br><?=$item->id?></div>
      ...
      <td class="products-list__td2" style="<?=(count($images_left) > 1) ? "padding-right: 53px;" : ""?>">
        ...
        <div class="products-list__img-wrapper">
          ...
          data-link="/product/view?id=<?=$item->id?>&source_id=<?=$source_id?>&comparisons=<?=$get_['filter-items__comparisons']?>&filter-items__profile=<?=$get_['filter-items__profile']?>"
          >
