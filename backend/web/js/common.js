...
lib.brief_on_hover = function(selector,prop,cb){
  ...
  if (typeof prop === 'object'){
    if (typeof prop.offset_left !== 'undefined') left = 215 + prop.offset_left;
    if (typeof prop.offset_right !== 'undefined') right = 15 + prop.offset_right;
    if (typeof prop.offset_top !== 'undefined') top = 15 + prop.offset_top;
...
lib.inputHandler = function(wrapper,selector,callback,options){
  ...
  if ( typeof _options.minSymbol === "undefined" ) _options.minSymbol = 0;
  if ( typeof _options.blurOn === "undefined" ) _options.blurOn = 1; // запускать когда пропадает фокус
  if ( typeof _options.startFocusOn === "undefined" ) _options.startFocusOn = 0;
  if ( typeof _options.delayMobile === "undefined" ) _options.delayMobile = 0;
  if ( typeof _options.delay === "undefined" ) _options.delay = 1000;
  ...
  */
  ...
  $(wrapper).on('blur', selector, function(evt) {
    var _this = $(this);
    if ( typeof _options.focusOut === "function" ) _options.focusOut(_this);
  });
  
  $(wrapper).on('focus', selector, function(evt) {
    var _this = $(this);
    if ( typeof _options.focusIn === "function") _options.focusIn(_this);
    str = $(_this[0]).val();
  });
  
  if (_options.startFocusOn) $(wrapper).on('focus', selector, processing);
  ...
  function processingKeyup(evt) { // keydown keyup - ! делает два одинаковых события
    ...
    if ( evt.keyCode === 37 || evt.keyCode === 38 || evt.keyCode === 39 || evt.keyCode === 40 ){
      ...
      if ( $(_this[0]).val().length >= _options.minSymbol ){
        if ( typeof _options.beforeStart_afterMinSymbol === "function") _options.beforeStart_afterMinSymbol(_this);
      } else {
        if ( typeof _options.beforeStart_beforeMinSymbol === "function") _options.beforeStart_beforeMinSymbol(_this);
      }
    }
  }
  ...
  function processing(evt) { // keydown keyup - ! делает два одинаковых события
    ...
    if ( (evt.keyCode === 13 && evt.type === 'keyup' && _this[0].localName !== 'textarea'/*&& str0.length >= 1*/) ){ // нажали на enter сразу запускаем (не ждем 1 или 4 секунд)
      ...
      if (isMobile() && $(_this[0]).val().length) { _this.blur() }
      return;
    }
    
    if (isMobile()) {
      delay = 4000; // для мобилы задержка 4 сек
      if (_options.delayMobile) delay = _options.delayMobile;
    } else {
      if (_options.delay) delay = _options.delay; else delay = 1000;
    }
    ...
    sto = setTimeout(function(){
      ...
      if ( block === false ) {
        ...
        if (_options.startFocusOn) callback(_this,evt);
        if (!block){
          if ( evt.keyCode !== 37 && evt.keyCode !== 38 && evt.keyCode !== 39 && evt.keyCode !== 40 ){ // не реагируем на стрелки
            if ( $(_this[0]).val().length >= _options.minSymbol || (evt.keyCode === 13 && $(_this[0]).val().length >= _options.minSymbol) || (evt.keyCode === 8 && $(_this[0]).val().length >= _options.minSymbol ) || (evt.keyCode === 46 && $(_this[0]).val().length >= _options.minSymbol) ) {
              ...
              callback(_this,evt);
              str = $(_this[0]).val();
              if (isMobile() && $(_this[0]).val().length && _this[0].localName !== 'textarea') { _this.blur() }
              //}
            }
          }
        }
...
lib.change_statistics_cnt = function($item, action){
  ...
  if (action === 'pre_match') pre_match = pre_match + 1;
  if (action === 'match') match = match + 1;
  if (action === 'mismatch') mismatch = mismatch + 1;
  
  
  if (name_before === 'pre_match') pre_match = pre_match - 1;
  if (name_before === 'match') match = match - 1;
  if (name_before === 'mismatch') mismatch = mismatch - 1;
  if (name_before === 'other') other = other - 1;
  if (name_before === 'nocompare') nocompare = nocompare - 1;
...
$(function(){
  ...
  // на правом товаре
  lib.brief_on_hover('.slider-item__border, .js__p-right',{'offset_left': 148},function($message,$this){
    let description = '<div>'+$this.data("text_brief")+'</div>';
    let img = $this.find('.slider-item__img').data('img');
    if (!img) img = $this.data('img');
    let img_html = '<img style="max-width: 100%;" src="'+img+'" />';
    
    $message.parents('.BRIEF').css('width', 320);
  
    let $node_id = $this.parents('.slider__slider-item').data('node_id');
    let $cloned = $this.parents('.js-slider-root').find('.slider__view-2 .slider__slider-item[data-node_id='+$node_id+'] .js-copy-for-view-1').clone();
  
    if (!$node_id) {
      $node_id = $this.data('node_id');
      $cloned = $('.js-slider-root').find('.slider__view-2 .slider__slider-item[data-node_id='+$node_id+'] .js-copy-for-view-1').clone();
    }
    
    if (!$cloned.length) $cloned = ''; else $cloned = $cloned.html();
    
    $message.html(description+img_html+$cloned)
